"""
Unit tests for the materials-only / service-only bid filter.

    python test_materials_filter.py
"""

from scanner import _is_materials_only, _is_service_only, _is_relevant
...
INSTALL_JOBS = [
    ...
    ("Remove existing carpet, install new carpet tile", "Contractor to strip and wax hard surfaces after install."),
]
...
def run():
    ...
    for title, desc in MATERIALS_ONLY:
        ...
        if _is_relevant(title, desc):
            ...
            fails += 1
    for title, desc in SERVICE_ONLY:
        if not _is_service_only(title, desc):
            print(f"FAIL (expected service-only): {title!r}")
            fails += 1
        if _is_relevant(title, desc):
            print(f"FAIL (_is_relevant should be False): {title!r}")
            fails += 1
    for title, desc in INSTALL_JOBS:
        if _is_materials_only(title, desc):
            print(f"FAIL (expected install job, got materials-only): {title!r}")
            fails += 1
        if _is_service_only(title, desc):
            print(f"FAIL (expected install job, got service-only): {title!r}")
            fails += 1

    # Scoring: materials_only / service_only are hard no-go regardless of factors
    for spec in ({"total_sqft": 30000, "materials_only": True},
                 {"raw_extract": {"materials_only": True}},
                 {"total_sqft": 30000, "service_only": True},
                 {"raw_extract": {"service_only": True}}):
        r = score_go_no_go({"is_relevant": True}, spec)
        if r != {"score": 0, "verdict": "no_go"}:
            print(f"FAIL scoring: {spec} -> {r}")
            fails += 1

    r = score_go_no_go({"is_relevant": True},
                       {"total_sqft": 30000, "materials_only": False, "service_only": False})
    if r["verdict"] != "go":
    ...
    if fails:
        print(f"\n{fails} failure(s)")
        raise SystemExit(1)
    print("all materials/service filter tests passed")
